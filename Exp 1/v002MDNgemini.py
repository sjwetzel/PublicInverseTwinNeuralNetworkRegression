# -*- coding: utf-8 -*-
"""
MDN version with multiplicative noise - Updated for Stability & Mode Inference
"""
...
def mdn_loss(n_components, output_dim):
    def loss_fn(y_true, y_pred):
        ...
        # FIX: Clip sigmas to prevent NaN from extreme variance or division by zero
        sigmas = tf.clip_by_value(tf.exp(log_sigmas), 1e-5, 1e5)

        y_true_exp = tf.expand_dims(y_true, axis=1)

        # Calculate exponent using the clipped sigmas
...
# =========================
# MDN Mode Inference (Replaces Sampling)
# =========================
def get_mode_from_mdn(y_pred, n_components, output_dim):
    """
    Instead of sampling randomly, this extracts the mean of the 
    most probable Gaussian component for a deterministic prediction.
    """
    logits = y_pred[:, :n_components]
    means = y_pred[:, n_components:n_components + n_components * output_dim]
    ...
    # Find the index of the most probable component for each sample
    best_components = np.argmax(logits, axis=-1)
    
    # Extract the mean of that specific component
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        # FIX: Use mode inference instead of sampling
        ...
        # Flatten if necessary for your custom utilities
        ...
        # Flatten y_test for MSE calculation if necessary
